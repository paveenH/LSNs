...
def main():
    ...
    parser.add_argument("--model-name", type=str, required=True, help="Model name, e.g., 'Llama3'")
    parser.add_argument("--model-size", type=str, required=True, help="Model size, e.g., '8B'")
    parser.add_argument("--model-path", type=str, required=True, help="Model name or path, e.g., 'mistralai/Mistral-7B-v0.3'")
    ...
    reps = model.extract_representations(pooling=args.pooling, batch_size=args.batch_size, dataset=dataset)
